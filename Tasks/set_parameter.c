...
void Vechicle_Parameter_Set(uint8_t *data)
{
	...
	vechicle = Bluetooth_Set_Keep_Dis(data,4);			//车距保持
	stSysPara.ucDistaceKeep = (uint8_t) vechicle;
	vechicle = Bluetooth_Set_TTC(data,5,6);
	stSysPara.ttc1 = (float)(vechicle / 10) + (float)(vechicle % 10);
	vechicle = Bluetooth_Set_TTC(data,6,7);				//第二刹车阈值
	stSysPara.ttc1 = (float)(vechicle / 10) + (float)(vechicle % 10);
...
{
	uint8_t send_data_correct[1024] = "USTART*SETPARAMETER*VEHICLE*1*USTOP";
	uint8_t send_data_wrong[1024] = "USTART*SETPARAMETER*VEHICLE*0*USTOP";
	uint32_t ret;
	uint8_t sencond_ret;
...
void Init_SetParameter(void)
{
	...
	stCanPara.can4rate = 1;//显示屏
	stCanPara.can5rate = 1;
...
{
	//速度参数
	FLASH_WriteWord(VEHICLE_SPEED_MODE,(uint32_t)stSpeedPara.uSingnalType);
...
void Init_Flash_Turn_Parameter(void)
{
	...
	FLASH_WriteWord(VEHICLE_TURN_BYPE,(uint32_t)stTurnPara.Source);
	FLASH_WriteWord(VEHICLE_TURN_STARTBYTE,(uint32_t)stTurnPara.ucByteNo);
	FLASH_WriteWord(VEHICLE_TURN_LSTARTBIT,(uint32_t)stTurnPara.ucLeftStartBit);
	FLASH_WriteWord(VEHICLE_TURN_LBITNUM,(uint32_t)stTurnPara.ucLeftBitLth);
	FLASH_WriteWord(VEHICLE_TURN_LEFFECTIVE,(uint32_t)stTurnPara.ucLeftValid);

	FLASH_WriteWord(VEHICLE_TURN_ID1,(uint32_t)stTurnPara.lId1);
	FLASH_WriteWord(VEHICLE_TURN_BYPE1,(uint32_t)stTurnPara.Source1);
	FLASH_WriteWord(VEHICLE_TURN_STARTBYTE1,(uint32_t)stTurnPara.ucByteNo1);
	FLASH_WriteWord(VEHICLE_TURN_RSTARTBIT,(uint32_t)stTurnPara.ucRightStartBit);
	FLASH_WriteWord(VEHICLE_TURN_RBITNUM,(uint32_t)stTurnPara.ucRightBitLth);
	FLASH_WriteWord(VEHICLE_TURN_REFFECTIVE,(uint32_t)stTurnPara.ucRightValid);
}
...
void Init_Flash_Sys_Parameter(void)
{
	...
	FLASH_WriteWord(VEHICLE_HMW1,(uint32_t)stSysPara.hmw1);
	FLASH_WriteWord(VEHICLE_HMW2,(uint32_t)stSysPara.hmw2);
}
...
void Init_Flash_CANRata_Parameter(void)
{
	...
	FLASH_WriteWord(CAN5_RATE_ADDRESS,(uint32_t)stCanPara.can5rate);
}
...
void Init_Flash_Urader_Position(uint8_t i)
{
	...
	//块7
	Urader_Company_Message[0].Uposition[1] = FRONT;
	Urader_Company_Message[0].Uposition[2] = FRONT;
	Urader_Company_Message[0].Uposition[3] = FLEFT;
	Urader_Company_Message[0].Uposition[4] = LEFT;
	Urader_Company_Message[0].Uposition[5] = LEFT;
}

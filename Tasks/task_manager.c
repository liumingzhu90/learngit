...
{
	//执行机构
	Set_LED6(stCanCommSta.Proportional_valve.status);
	//毫米波雷达
	Set_LED7(stCanCommSta.stRadar.status);
	//车can
	Set_LED5(stCanCommSta.stVehicle.status);
	//相机
	Set_LED4(stCanCommSta.stCamera.status);
	//车速
	Set_LED3(1);
	//无线(4G)
	Set_LED2(stCanCommSta.stWireless.status);
	//超声波雷达
	Set_LED1(stCanCommSta.stHRadar.status);
}

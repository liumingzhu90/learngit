...
//		BreakFlag = 1;
		...
		vehicleDevice_info[4] |= 1<<4;//(stVehicleParas.RightFlagTemp&0x01) << 4;
		// 获取左转向灯状态--字节5 D4	【0：否]、【1：是】
		vehicleDevice_info[4] |= (stVehicleParas.LeftFlagTemp&0x01) << 3;
		// 获取刹车灯状态--字节5 D3	【0：否]、【1：是】
		vehicleDevice_info[4] |= 1<<2;//(stVehicleParas.BrakeFlag&0x01) << 2;
		// 获取档位状态--字节5 D2-D1	【0：无效】、【1：N档】、【2：D档】、【3：R档】
		vehicleDevice_info[4] |= 2;//stVehicleParas.Car_Gear&0x03;
		// 获取故障码--字节6
...
void UpLoad_Car_Body_Info()
{
	...
	if(SystemtimeClock - upload_CarBody_time > 10){	// 与其他发送延时10ms,否则GPS设备无返回信息号
		...
		if(vechicleBodyInfo_flag)
		{
			...
			// 获取档位状态--字节5，D2-D1
			carBody_info[4] |=  stVehicleParas.Car_Gear & 0x03;
			// 获取故障码--字节6，D8-D1
			...
			// 获取油门踏板开度--字节7，无效0XFF
			carBody_info[6] = stVehicleParas.ThrottleOpening;
			// 获取刹车踏板开度--字节8，无效0xFF
			carBody_info[7] = stVehicleParas.BrakeOpening;
			// 获取方向盘角度(LSB)|(MSB)--字节9、字节10，无效0x7FF
			carBody_info[8] = stSWAParas.SWADegree & _0XFF;
			carBody_info[9] = stSWAParas.SWADegree >> 8;
			// 备用--字节11，0xFF
...
//	BreakFlag = 1;
	...
	earlyWarning_info[18] |= (stVehicleParas.Car_Gear&0x03) << 4;
	// 获取事件的开始与结束--字节19 D4 【0：结束】、【1：开始】
	earlyWarning_info[18] |= 0;
	// 获取事件分类--字节19 D3-D1	【1：FCW】、【2：HMW】、【3：LDW】、【4：AEB】
	earlyWarning_info[18] |= 4;

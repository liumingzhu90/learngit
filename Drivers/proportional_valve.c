...
extern uint8_t Camera_Info_Push_Flag;
extern uint8_t cipv_check;
...
//	uint8_t turnright = 0;
	...
	//fprintf(USART1_STREAM,"wheel=%0.2f,canspeed=%0.2f\r\n",Get_Wheel_Speed()/10.0,stVehicleParas.fVehicleSpeed);
	//stVehicleParas.fVehicleSpeed = Get_Wheel_Speed()/10.0;// Error
	ret_pressure = AEB_CMS_Break_control(Camera_Info_Push_Flag,&obstacle_Basic_data,&obstacle_cipv_data, &stVehicleParas, &camera_data, Urader_Company_Message);
	if(ret_pressure > 0.0){
		stVehicleParas.BreakState = 0x01;
	}
	// Break by Uradar
	//Uradar_test(Urader_Company_Message);
	if((0 < stVehicleParas.fVehicleSpeed) && (stVehicleParas.fVehicleSpeed < 25))
	{
		//upressure = SSS_Break_control(stVehicleParas,Urader_Company_Message);
	}
	if(upressure != 0)
	{
		//ret_pressure = upressure;
		stVehicleParas.BreakState = 0x03;
	}
	// No break
	if(ret_pressure == 0){
		stVehicleParas.BreakState = 0x00;
	}
	...
	tx_valve_control.data[0] = (uint8_t)(m_pressure & 0x00FF);
	tx_valve_control.data[1] = (uint8_t)((m_pressure & 0xFF00) >> 8);
	*tx_frame = tx_valve_control;
	return pdTRUE;
